...
class BayesGeomOpt:
    ...
    def grid_search_convergence_plot(self, bo_history, bounds, best_idx, grid_search, plot):
        ...
        if len(params[0])==1:
            ...
            ax[0].axhline(y=best_param, color='green', linestyle='dashed')
            ...
            ax[0].scatter([param[0] for param in params], [param[1] for param in params], c=np.arange(len(params)), cmap='RdYlGn')
            cbar = plt.colorbar(c, cax=ax[0])
            cbar.set_label('Score', rotation=270, labelpad=15)
            best_param = params[best_idx]
            ax[0].scatter(best_param[0], best_param[1], c='white', s=100, label='best', alpha=0.3)
            ax[0].legend()
            ax[1].plot(np.minimum.accumulate(scores))
            ax[1].set_xticks(np.arange(len(scores), step=5))
            ax[1].set_xlabel('Iteration')
