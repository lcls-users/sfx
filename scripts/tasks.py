...
def test_serv_client(config):
    ...
    command += f"; /sdf/home/n/nathfrn/mfxp23120/scratch/nathfrn/env_nath/bin/mpirun -n {ncores} python {client_path} -e {exp} -r {run} -d {det_type} --start_offset {start_offset} --num_images {num_images} --loading_batch_size {loading_batch_size} --num_components {num_components} --batch_size {batch_size} --path {path} --tag {tag}"
