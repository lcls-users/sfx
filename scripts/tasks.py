...
def t_sne(config):
    ...
    command += f"; python {client_path} --filename {filename} --num_images {num_images} --loading_batch_size {loading_batch_size} --num_tries {num_tries} --threshold {threshold} --num_runs {num_runs}"

    js = JobScheduler(os.path.join(".", f't_snes_{num_images_str}.sh'),queue = 'ampere', ncores=  1, jobname=f't_snes_{num_images_str}',logdir='/sdf/home/n/nathfrn/btx/scripts',account='lcls',mem = '200G',num_gpus = 4) ##
    js.write_header()
